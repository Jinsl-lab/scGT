import os
from collections import defaultdict # 一个字典的子类，它提供了一种在访问字典中不存在的键时自动创建默认值的方法
...
# 数据分割函数
def rand_divide_idx(label, metadata, train_prop=.8, valid_prop=.1, ignore_negative=True):
    """ randomly splits label into train/valid/test splits
        随机划分训练集、测试集、验证集的标签，返回对应的索引
        请注意，由于是用于处理多组学数据，因此查询数据集数据的标签不能用于求loss，只有参考集的可以。这要单独设置一下。用metadata
    """
#    if ignore_negative:
#       labeled_nodes = torch.where(label != -1)[0] # 获取label！=1的行索引  如果存在参考数据存在unknown标签，可以用这个
#    else:
#        labeled_nodes = label

    # 获取reference与query的索引
    label_reference = torch.where(metadata == 1)[0]
    label_query = torch.where(metadata == 2)[0]

    n = label_reference.shape[0] # 总共有多少个标签，或者说样本数
    train_num = int(n * train_prop) # 训练样本的数目
    valid_num = int(n * valid_prop) # 测试样本的数目

    perm = torch.as_tensor(np.random.permutation(n)) # 生成一个从0到n-1的随机排列数组，并转化为tensor
...
#    train_idx = torch.cat((train_idx, label_query),dim = 0)
    valid_idx = label_reference[val_indices]
    test_idx = label_reference[test_indices]
    query_idx = label_query

    return train_idx, valid_idx, test_idx, query_idx
...
def rand_train_test_idx(label, train_prop=.5, valid_prop=.25, ignore_negative=True):
    ...
    """
    if ignore_negative:
        labeled_nodes = torch.where(label != -1)[0] # 获取满足条件的行索引
    else:
        labeled_nodes = label

    n = labeled_nodes.shape[0] # 总共有多少个标签，或者说样本数
    train_num = int(n * train_prop) # 训练样本的数目
    valid_num = int(n * valid_prop) # 测试样本的数目
...
def load_fixed_splits(data_dir, dataset, name, protocol): # 数据集的划分，返回列表
    splits_lst = []
...
def class_rand_splits(label, label_num_per_class): # 基于类别的随机划分，返回索引
    train_idx, non_train_idx = [], []
    idx = torch.arange(label.shape[0])
    class_list = label.squeeze().unique() # 标签中的类别
    valid_num, test_num = 500, 1000
    for i in range(class_list.shape[0]):
        c_i = class_list[i]
        idx_i = idx[label.squeeze() == c_i]
        n_i = idx_i.shape[0]
        rand_idx = idx_i[torch.randperm(n_i)] # 打乱顺序
...
def even_quantile_labels(vals, nclasses, verbose=True): # 按分位数划分
...
def to_planetoid(dataset): # 用于将给定的 NCDataset 转换为 H2GCN Planetoid 格式的数据集
    """
...
def gen_normalized_adjs(dataset): # 获得用D处理后的A
...
def adj_mul(adj_i, adj, N): # 稀疏矩阵的乘法
...
def get_gpu_memory_map(): # 获取当前 GPU 的内存使用情况
