...
filter4_weight_linux <- function(dir, n_ = 12) {
  ...
  p_value <- foreach(g = 1:n1, .combine = c, .packages = 'base', .export = c('indices', 'label1')) %dopar% {
    adj_nodes <- indices[indices[, 1] == g, 2]  # 找到节点g的邻居
    n_adj <- length(adj_nodes)  # 邻居数目
    p <- 0.0  # 默认值
    if (n_adj > 0) {
      type_count <- table(label1[adj_nodes])  # 统计邻接节点类别的数量
      p <- type_count[label1[g]] / n_adj  # 计算当前节点类别占比
    }
    if (g %% 1000 == 0) {
      cat("Processed node: ", g, " at ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n", file = log_file, append = TRUE)  # 输出到日志文件
      # flush(stdout())  # 强制刷新输出
    }
    p  # 返回当前节点的p值
  }
  ...
  saveRDS(data2_celltype_pre, "data2_celltype_pre.rds")
