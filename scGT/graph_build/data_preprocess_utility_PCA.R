### 这里的内图由PCA求取得到，速度加快，效果有时更好，k取10时连边已经很多了 ###
## asap上内图k取8足以，10也行
...
metrics <- function(lab1,inter_graph,clusters){
  ...
  eval1 <- sapply(names(table(hc)),function(name){
    ps1 <- which(hc==name)
    foo <- table(lab1[inter_graph[which(inter_graph[,2]%in%ps1),1]])
...
GenerateGraph <- function(Dat1,Dat2,Lab1,K,check.unknown,dir){
  ...
  objects1 <- lapply(objects,function(obj){
    ...
    obj <- FindVariableFeatures(obj,
                                ...
                                nfeatures = 5000)
    print(paste("------------------------完成+1: ", Sys.time()))
    return(obj)})
    
  features <- SelectIntegrationFeatures(object.list = objects1, nfeatures = 5000)
  objects1 <- lapply(X = objects1, FUN = function(x) {
    x <- ScaleData(x, features = features)
    x <- RunPCA(x, features = features)
  })
    
  print(paste("------------------------预处理结束: ", Sys.time()))
  # 保存data
  # gene1 <- na.omit(objects1[[1]]@assays[["Data1"]]@meta.data[["var.features"]])
  # gene2 <- na.omit(objects1[[2]]@assays[["Data2"]]@meta.data[["var.features"]])
  # gene <- union(gene1, gene2)
  # rownames(Dat1) <- gsub("_", "-", rownames(Dat1), fixed = TRUE)
  # rownames(Dat2) <- gsub("_", "-", rownames(Dat2), fixed = TRUE)
  # write.csv(t(Dat1[gene,]),file=paste0(dir,'/Data1.csv'),quote=F,row.names=T)
  # write.csv(t(Dat2[gene,]),file=paste0(dir,'/Data2.csv'),quote=F,row.names=T)
  
  print(paste("------------------------开始计算图间连接: ", Sys.time()))
  #'  Inter-data graph  
  object.nn <- FindIntegrationAnchors(object.list = objects1,k.anchor=50, anchor.features = features, reduction = "rpca")
  ...
  if (check.unknown){
    obj <- objects1[[2]]
    obj <- RunPCA(obj, features = VariableFeatures(object = obj),npcs=10,verbose=F)
  ...
  print(paste("------------------------开始计算图内连接: ", Sys.time()))
  d1 <- FindNeighbors(objects1[[1]], dims = 2:50, k.param = 10, return.neighbor = TRUE)
  d1.knn <- d1@neighbors$Data1.nn@nn.idx
  d1.grp <- findMNN(d1.knn)-1
  
  d2 <- FindNeighbors(objects1[[2]], dims = 2:50, k.param = 10, return.neighbor = TRUE)
...
normalize_data <- function(count.list){
  ...
  for ( i in 1:length(count.list)){
    norm.list[[i]] <- as.matrix(Seurat:::NormalizeData.default(Matrix::Matrix(as.matrix(count.list[[i]]),sparse = T),verbose=F))
    #' select variable features
    print("begin hvf")
    hvf.info <- Seurat:::FindVariableFeatures.default(Matrix::Matrix(as.matrix(count.list[[i]]),sparse = T),selection.method='vst',verbose=F)
...
#' @examples: load count.list and label.list from folder "example_data"
#' save_processed_data(count.list,label.list)
save_processed_data <- function(count.list,label.list,Rgraph=TRUE,check_unknown=FALSE,dir="input"){
  print(paste("------------------------begin: ", Sys.time()))
  dir.create(dir);
  # write.csv(t(count.list[[1]]),file=paste0(dir,'/Data1.csv'),quote=F,row.names=T)
  # write.csv(t(count.list[[2]]),file=paste0(dir,'/Data2.csv'),quote=F,row.names=T)
#   rna <- CreateSeuratObject(count[[1]], assay = "RNA")
#   atac <- CreateSeuratObject(count[[2]], assay = "atac")
#   rna$cell_type <- label[[1]]
#   atac$cell_type <- label[[2]]
